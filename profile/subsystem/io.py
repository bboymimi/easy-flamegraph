...
tsp = []
order = []
print("%-9s %-6s %-16s %10s %s %-3s" % ("TIME", "PID", "COMM", "LATns", "TS",
                                        "ORDER"))
...
def print_event(cpu, data, size):
    ...
    print("%-9s %-6d %-16s %10.2f %-5s %-3d" % (strftime("%H:%M:%S"),
                                                event.pid,
...
def plot_io(view):
    p = figure(title=view, plot_width=400, plot_height=200, tools=TOOLS,
